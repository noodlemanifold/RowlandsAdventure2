 ...
 * 
 */
public class UDPClientInfo<K> implements IClientInfo {
	private InetAddress addr; // client's address
	private int port; // port client is listening on
	private UDPServerSocket<K> serverSocket; // the current server socket
	...
	public UDPClientInfo(InetAddress address, int port,
			UDPServerSocket<K> serverSocket) {
		addr = address;
		this.port = port;
	 ...
	 */
	public UDPServerSocket<K> getServerSocket() {
		return serverSocket;
	}
