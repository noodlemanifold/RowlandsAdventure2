...
public class Camera
{
	...
	public void lookAt(Vector3f target) { lookAt(target.x(), target.y(), target.z(),0f,0f,0f); }
	...
	public void lookAt(GameObject go, Vector3f up) { lookAt(go.getWorldLocation(),up); }
	...
	{	setN((new Vector3f(x-location.x(), y-location.y(), z-location.z())).normalize());
		...
		if ((n.equals(0,1,0)) || (n.equals(0,-1,0)))
			...
			u = (new Vector3f(copyN.cross(ux,uy,uz))).normalize();
