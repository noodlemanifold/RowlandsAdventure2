...
public abstract class ObjShape
{
	...
	private float[] vertices, texCoords, normals, tangents, boneWeights, boneIndices;
	private int vertexBuffer, texCoordBuffer, normalBuffer, tangentBuffer, boneWeightBuffer, boneIndicesBuffer;
	...
	protected void setTangents(Vector3f[] t)
	{	tangents = new float[t.length*3];
		for (int i=0; i<numVertices; i++)
		{	tangents[i*3]   = (float) t[i].x();
			tangents[i*3+1] = (float) t[i].y();
			tangents[i*3+2] = (float) t[i].z();
		}
	}
	...
	protected void setTangentBuffer(int b) { tangentBuffer = b; }
	...
	/** engine use only. */
	public int getTangentBuffer() { return tangentBuffer; }
	/** engine use only. */
	...
	/** returns a reference to the float array containing all of the tangent vectors for this ObjShape */
