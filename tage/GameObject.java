...
public class GameObject
{
	...
	private JmeBulletPhysicsObject physicsObject;
	private boolean isTerrain = false;
	private float terrrainScale = 0f;
	...
	/** returns the Secondary TextureImage associated with this GameObject */
	public TextureImage getTextureImage2() { return texture2; }

	/** assigns the Secondary TextureImage to this GameObject */
	public void setTextureImage2(TextureImage tex) { texture2 = tex; }
	...
	public void setHeightMap(TextureImage tex) { heightMap = tex; isTerrain = true; terrrainScale = 1f;}
	...
	/** returns the scaling of the vertical displacement of the terrain */
	public float getTerrainScale() { return terrrainScale; }

	/** sets the scaling of the vertical displacement of the terrain */
	public void setTerrainScale(float h) { terrrainScale = h; }
	...
	*/
	...
	/** Orients this GameObject so that it faces a location specified in a Vector3f with a specified up direction also as a Vector3f*/
	public void lookAt(Vector3f target, Vector3f up) { lookAt(target.x(), target.y(), target.z(), up.x(), up.y(), up.z()); }

	/** Orients this GameObject so that it faces a specified (x,y,z) location */
	public void lookAt(float x, float y, float z) {lookAt(x, y, z, 0f, 1f, 0f);}

	/** Orients this GameObject so that it faces a specified (x,y,z) location with up direction (ux,uy,uz)*/
	public void lookAt(float x, float y, float z, float ux, float uy, float uz)
	{	Vector3f fwd, l;
		l = getWorldLocation();
		fwd = (new Vector3f(x-l.x(), y-l.y(), z-l.z())).normalize();
		lookTowards(fwd, new Vector3f(ux,uy,uz));
	}

	/** Orients this GameObject so that it faces a direction specified in a Vector3f with a specified up direction also as a Vector3f*/
	public void lookTowards(Vector3f dir, Vector3f upDir){
		Vector3f right, up, copyFwd, copyRight;
		copyFwd = new Vector3f(dir);
		if ((dir.equals(0,1,0)) || (dir.equals(0,-1,0)))
			right = new Vector3f(1f,0f,0f);
		else
			right = (new Vector3f(copyFwd.cross(upDir))).normalize();
		copyRight = new Vector3f(right);
		up = (new Vector3f(copyRight.cross(dir))).normalize();
		localRotation.identity();
		localRotation.setColumn(0, new Vector4f(right.negate(), 0f));
		localRotation.setColumn(1, new Vector4f(up, 0f));
		localRotation.setColumn(2, new Vector4f(dir, 0f));
		update();
	}

	// ------------------ Rotation Methods ------------------------------

	/** Rotates GameObject around vertical world axis by X Radians*/
	public void yaw(float rad){
		Matrix4f yawRot = (new Matrix4f()).rotation((float) (rad), 0, 1, 0)
				.mul(getLocalRotation());
		setLocalRotation(yawRot);
	}

	/** Rotates GameObject around right local axis by X Radians*/
	public void pitch(float rad){
		Vector3f right = getLocalRightVector();
		Matrix4f pitchRot = (new Matrix4f())
				.rotation((float) (rad), right.x, right.y, right.z)
				.mul(getLocalRotation());
		setLocalRotation(pitchRot);
	}

	/** Rotates GameObject around forward local axis by X Radians*/
	public void roll(float rad){
		Vector3f forward = getLocalForwardVector();
		Matrix4f rollRot = (new Matrix4f())
				.rotation((float) (rad), forward.x, forward.y, forward.z)
				.mul(getLocalRotation());
		setLocalRotation(rollRot);
	}
	...
	public void setPhysicsObject(JmeBulletPhysicsObject p) { physicsObject = p; }

	/** gets the physics object associated with this GameObject */
	public JmeBulletPhysicsObject getPhysicsObject() { return physicsObject; }
