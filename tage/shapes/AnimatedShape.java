...
public final class AnimatedShape extends ObjShape
{
	...
	private HashMap<Edge,EdgeNeighbors> edgeMap = new HashMap<Edge,EdgeNeighbors>();
	...
	private float baseFrameRate = 24f;//default frame rate in blender

	/** Specifies filenames for the model (with extension "rkm") and the skeleton (with extension "rks"). */
	public AnimatedShape(String meshPath, String skelPath, float baseFrameRate)
	{
		...
		Vector3f[] verticesV = new Vector3f[triCount*3*2];
		Vector2f[] texCoordsV = new Vector2f[triCount*3*2];
		Vector3f[] normalsV = new Vector3f[triCount*3*2];
		Vector3f[] boneWeightsV = new Vector3f[triCount*3*2];
		Vector3f[] boneIndicesV = new Vector3f[triCount*3*2];
		...
		Vector3f nnw = new Vector3f(0f,0f,0f);
		Vector3f nni = new Vector3f(0f,0f,0f);

		for (int i = 0; i < triCount; i++)
		{
			verticesV[i*3*2+0] = new Vector3f(
			...
			verticesV[i*3*2+1] = nnv;
			verticesV[i*3*2+2] = new Vector3f(
			...
			verticesV[i*3*2+3] = nnv;
			verticesV[i*3*2+4] = new Vector3f(
			...
			normalsV[i*3*2+1] = nnn;
			normalsV[i*3*2+2] = new Vector3f(
			...
			normalsV[i*3*2+3] = nnn;
			normalsV[i*3*2+4] = new Vector3f(
			...
			normalsV[i*3*2+5] = nnn;

			boneWeightsV[i*3*2+0] = new Vector3f(
			...
			boneWeightsV[i*3*2+1] = nnw;
			boneWeightsV[i*3*2+2] = new Vector3f(
			...
			boneWeightsV[i*3*2+3] = nnw;
			boneWeightsV[i*3*2+4] = new Vector3f(
			...
			boneWeightsV[i*3*2+5] = nnw;

			boneIndicesV[i*3*2+0] = new Vector3f(
			...
			boneIndicesV[i*3*2+1] = nni;
			boneIndicesV[i*3*2+2] = new Vector3f(
			...
			boneIndicesV[i*3*2+3] = nni;
			boneIndicesV[i*3*2+4] = new Vector3f(
		...
		for (int i = 0; i < triCount; i++) {
			...
			if (a1 >=0){
				...
				boneWeightsV[i1_2] = boneWeightsV[a1];
				boneIndicesV[i1_2] = boneIndicesV[a1];
			}
			if (a2 >=0){
				verticesV[i2_3] = verticesV[a2];
				texCoordsV[i2_3] = texCoordsV[a2];
				normalsV[i2_3] = normalsV[a2];
				boneWeightsV[i2_3] = boneWeightsV[a2];
				boneIndicesV[i2_3] = boneIndicesV[a2];
			}
			if (a3 >=0){
				verticesV[i3_1] = verticesV[a3];
				texCoordsV[i3_1] = texCoordsV[a3];
				normalsV[i3_1] = normalsV[a3];
				boneWeightsV[i3_1] = boneWeightsV[a3];
				boneIndicesV[i3_1] = boneIndicesV[a3];
			}
		}

		setNumVertices(triCount*3*2);
	...
	public void updateAnimation(float deltaTime){
		updateAnimation(deltaTime, 1f);
	}

	/** The game application should call this function at each frame (i.e., in update()) for each animated shape. */
	public void updateAnimation(float deltaTime, float animSpeedFactor)
	{	update(deltaTime, animSpeedFactor);
	...
	private void update(float deltaTime, float animSpeedFactor)
	{	if (curAnimation != null && !curAnimPaused && curAnimSpeed != 0.0f)
		{	curLerpedAnimFrame += curAnimSpeed * deltaTime * baseFrameRate * animSpeedFactor;
